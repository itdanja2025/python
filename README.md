...
문제 4: 특정 문자의 개수 세기
사용자로부터 긴 문장을 입력받고, 그 문장에 알파벳 'a'가 몇 개 포함되어 있는지 출력하시오.

문제 5: 숫자 형태 확인
사용자로부터 입력을 받아 해당 입력값이 숫자로만 이루어져 있는지 여부(True/False)를 출력하시오. (힌트: .isdigit() 메서드 활용)

문제 6: 두 점수의 합격 판별
두 개의 점수를 입력받아 총점이 120점 이상이고 각 점수가 모두 50점 이상이면 "합격", 아니면 "불합격"을 출력하시오.

문제 7: 큰 수 찾기 
서로 다른 두 정수를 입력받아 더 큰 숫자를 찾아 "[숫자]가 더 큰 수입니다." 형식으로 출력하시오.
...
문제 11: 배수 판별 
입력받은 정수가 3의 배수이면서 동시에 5의 배수이면 True, 아니면 False를 출력하시오.
...
문제 14: 양수, 음수, 제로 판별
정수 하나를 입력받아 양수이면 "양수", 음수이면 "음수", 0이면 "0"을 출력하시오.

문제 15: 성적 등급 산출
점수를 입력받아 90점 이상은 "A", 80점 이상은 "B", 70점 이상은 "C", 그 미만은 "F"를 출력하시오.

문제 16: 윤년 판별기
연도를 입력받아 4의 배수이면서 100의 배수가 아니거나, 400의 배수이면 "윤년", 아니면 "평년"을 출력하시오.

문제 17: 세 수 중 최댓값 구하기
서로 다른 세 정수를 입력받아 그중 가장 큰 수를 출력하시오.
