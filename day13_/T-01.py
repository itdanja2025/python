...
df_vertical = pd.concat([df_info, df_score], axis=0, ignore_index=True)  # 인덱스를 0부터 다시 생성
# 세로 연결 결과 출력
print(df_vertical)  # 연결된 데이터 출력

# 가로 연결: 열(column)을 옆으로 붙임
df_horizontal = pd.concat([df_info, df_score], axis=1)  # 인덱스 기준으로 옆에 붙음
# 가로 연결 결과 출력
...
# # 기존 데이터프레임에 새로운 열 추가
df_info['NewScore'] = new_score  # 인덱스를 기준으로 값이 들어감
# Series 추가 결과 출력
print(df_info)  # 결과 출력
