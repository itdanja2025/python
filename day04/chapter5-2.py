# 재귀함수 이란 ? 현재 실행 중인 (자신의)함수를 다시 호출 하는것  
# [1] 반복문으로 팩토리얼 구하기.
def func1( n ) : 
    output = 1 
...
# [2] 재귀함수로 팩토리얼 구하기 
# func2( 5 ) -> 5 * 재귀    (안끝남)                                                5 * 3 * 2 * 1 * 1
# func2( 4 ) ->     4 * 재귀    (안끝남)                                        4 * 3 * 2 * 1 * 1 
# func2( 3 ) ->         3 * 재귀    (안끝남)                                3 * 2 * 1 * 1
# func2( 2 ) ->             2 * 재귀    (안끝남)                        2 * 1 * 1 
# func2( 1 ) ->                 1 * 재귀    (안끝남)                1 * 1       
...
# [3] 피보나치 수열 1 , 1번째수열 : 1 , 2번째 수열 : 2 , n번째 열 : n-1수열 + n-2수열 
...
def 문제( 남은사람수 , 최소사람수 ) :           # 100 , 2  
    key = str( [남은사람수, 최소사람수 ] )      
    if key in memo : 
        return memo[key]                    # 메모/저장 된 값이 기존 값 사용 
    if 남은사람수 < 0 : 
        return 0
    if 남은사람수 == 0 :
        return 1 
    ...
    count = 0       # 인원수 
    for i in range( 최소사람수 , 앉힐수있는최대사람수 + 1 ) : # 2부터 10까지 1씩증가 반복 
        count += 문제( 남은사람수 - i , i )                # 남은사람수에 i 만큼 제외하고  , i 대입  
